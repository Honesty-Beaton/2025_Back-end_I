...
// Reading a file
fs.readFile('./files/file1.txt', 'utf8', (err, data) => { // Asynchronously reads the entire contents of 'text.txt'.
  if (err) throw err; 
...
fs.writeFile(filePath, 'Hello, this is a test to write file!', (err) => { // Asynchronously writes to the constructed file path.
  if (err) throw err;
...
// Appending to a file
fs.appendFile('./files/file2.txt', '\nThis is appended data.', (err) => { // Asynchronously appends data to 'newfile.txt'.
  if (err) throw err; 
  console.log('Data appended successfully.');  
}); // Adds the provided string to the end of the file. '\n' starts a new line.

// Renaming a file
fs.rename('./files/test.txt', 'renamedfile.txt', (err) => { // Asynchronously renames 'newfile.txt' to 'renamedfile.txt'.
  if (err) throw err; 
